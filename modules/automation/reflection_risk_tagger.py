...
def _kw_score(txt: str) -> tuple[float, str]:
    ...
    if any(k in t for k in RISK_KEYWORDS["HIGH"]):
        lvl = "HIGH"
        s = 0.9
    elif any(k in t for k in RISK_KEYWORDS["MED"]):
        lvl = "MED"
        s = 0.6
    elif any(k in t for k in RISK_KEYWORDS["LOW"]):
        lvl = "LOW"
        s = 0.3
    if RISK_HINTS.search(t):
        s = max(s, 0.7)
        lvl = "HIGH" if s >= 0.8 else ("MED" if s >= 0.5 else lvl)
    return s, lvl
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--include-reflections", action="store_true", help="scan data/reflections/*.json also"
    )
    ...
    if args.include_reflections and REF_DIR.exists():
        for p in sorted(REF_DIR.glob("*.json")):
            try:
                ...
                items.append(
                    {"text": text, "ts": j.get("ts") or j.get("timestamp"), "source": "reflection"}
                )
            except:
                pass
    ...
    rep = {
        ...
        "counts": {
            "total": len(tagged),
            "HIGH": sum(x["risk_level"] == "HIGH" for x in tagged),
            "MED": sum(x["risk_level"] == "MED" for x in tagged),
            "LOW": sum(x["risk_level"] == "LOW" for x in tagged),
        },
        "top_high": top_high,
    }
    (REPORTS / "reflection_risk_report.json").write_text(
        json.dumps(rep, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    # md
    lines = [
        f"# Reflection Risk Report",
