...
def call(prompt:str, *, complexity:str="low", **kw)->str:
    ...
    if h in cache and (datetime.datetime.utcnow()-datetime.datetime.fromisoformat(cache[h]['ts'])).total_seconds()<86400:
        return cache[h]['resp']
    ...
    cache[h]={"resp":resp,"ts":datetime.datetime.utcnow().isoformat()}; _cache_save(cache); return resp
