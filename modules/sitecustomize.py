...
def _now(): return time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())
def _w(path, obj):
...
try:
    _w(CURSOR_LOOP, {"status":"STARTED","stage":_stage,"started_at":_now(),"pid":os.getpid(),"argv":sys.argv})
...
if _fail_flag.exists():
    try: _w(CURSOR_LOOP, {"status":"FAILED","stage":_stage,"failed_at":_now(),"reason":"force_fail.flag"})
    finally: sys.exit(1)
...
def _finalize_success():
    ...
    if REPORTS.exists():
        ...
        if files:
            ...
            try: _w(CURSOR_REPORT, {"latest": latest, "updated_at": _now()})
            except Exception: pass
    try: _w(CURSOR_LOOP, {"status":"SUCCEEDED","stage":os.getenv("VELOS_STAGE") or _stage,"ended_at":_now(),"latest_report":latest})
    except Exception: pass

def _finalize_failure(exc_type, exc, tb):
    try:
        _w(CURSOR_LOOP, {"status":"FAILED","stage":os.getenv("VELOS_STAGE") or _stage,"failed_at":_now(),
                         "error":"".join(traceback.format_exception(exc_type, exc, tb))[:2000]})
    except Exception:
        pass

atexit.register(_finalize_success)
def _exh(t, e, b): _finalize_failure(t,e,b); sys.__excepthook__(t,e,b)
sys.excepthook = _exh
