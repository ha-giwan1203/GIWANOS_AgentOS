...
def db_open(path=None):
    ...
    if path is None:
        ...
        # Security validation for provided path
        try:
            from .security_validator import sanitize_path, validate_path

            if not validate_path(path):
                print(f"⚠️ Invalid database path provided, sanitizing: {path}")
                sanitized_path = sanitize_path(path)
                if sanitized_path:
                    db = sanitized_path
                else:
                    print("⚠️ Path sanitization failed, using default")
                    db = os.environ.get("VELOS_DB_PATH", r"C:\giwanos\data\memory\velos.db")
            else:
                db = path
        except ImportError:
            print("⚠️ Security validator not available for path validation")
            db = path
