...
class VelosPathManager:
    ...
    def _initialize_root(self):
        ...
        self._root_path = (
            os.environ.get("VELOS_ROOT_PATH") or
            os.environ.get("GIWANOS_ROOT") or  # Legacy support
            (self.DEFAULT_WINDOWS_ROOT if os.name == 'nt' else self.DEFAULT_LINUX_ROOT)
        )
...
if __name__ == "__main__":
    ...
    test_paths = [
        get_db_path() if "get_db_path" in locals() else get_data_path("memory/velos.db") if "get_data_path" in locals() else "C:/giwanos/data/memory/velos.db",
        get_config_path("settings.yaml") if "get_config_path" in locals() else "C:/giwanos/configs/settings.yaml",
        "/some/absolute/path",
        "relative/path"
    ]
