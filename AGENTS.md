...
0. **(진행상황) Claude 앱 채팅창 직접 입력**:
   ...
   - 호출 명령: `python 90_공통기준/업무관리/codex_claude_channel/auto_reply.py --target claude "[Codex 진행] <현재 단계 / 다음 행동>"`
   - Codex 앱/샌드박스 셸에서 GUI 자동입력을 실행할 때는 `require_escalated` GUI 권한으로 실행한다. 일반 실행에서 `GUI_ACCESS_UNAVAILABLE` 또는 `window not found`가 나오면 즉시 같은 명령을 `require_escalated`로 1회 재시도한 뒤 판정한다
   - exit code: `0` PASS, `1` FAIL, `10` SKIP, `11` ENTER_FAILED — 그 외(`argparse` 2)는 호출자 인자 오류
...
1. **(A 메인) Claude 앱 채팅창 직접 입력**:
   ...
   - 호출 명령: `python 90_공통기준/업무관리/codex_claude_channel/auto_reply.py --target claude "[Codex 완료] <한 줄 요약>"`
   - Codex 앱/샌드박스 셸에서 GUI 자동입력을 실행할 때는 `require_escalated` GUI 권한으로 실행한다. 일반 실행에서 `GUI_ACCESS_UNAVAILABLE` 또는 `window not found`가 나오면 즉시 같은 명령을 `require_escalated`로 1회 재시도한 뒤 판정한다
   - exit code: `0` PASS, `1` FAIL, `10` SKIP, `11` ENTER_FAILED — 그 외(`argparse` 2)는 호출자 인자 오류
   ...
   - `require_escalated` 재시도 후에도 computer-use grant 없거나 Claude 창 식별 실패 시 우회하지 않고 B를 실행하며, 실패 원인과 재시도 결과를 `auto_reply.log`와 `review.md`에 남긴다
2. **(B 안전망 fallback) `90_공통기준/업무관리/검토기록/runs/<YYYYMMDD_task-slug>/review.md` 작성**:
   - 진단·diff·재빌드·검증 결과 전체 기록
   - mtime 갱신이 Claude 폴링 신호

3. **(Claude → Codex 작업 위임) Codex 앱 채팅창 직접 입력**:
   - 호출 명령: `python 90_공통기준/업무관리/codex_claude_channel/auto_reply.py --target codex "<지시문>"`
   - exit code: `0` PASS, `1` FAIL, `10` SKIP, `11` ENTER_FAILED — 그 외(`argparse` 2)는 호출자 인자 오류
   - Claude는 computer-use 5단계 대신 Bash 한 줄 호출로 통일한다
   - 좌표 의존 제거. 창 자동 탐색
