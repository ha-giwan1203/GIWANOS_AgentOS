...
- **자동화 명제 layer 폐기 (세션136)**: 이전 "routine 즉시 실행 원칙" 블록은 폐기됨. 사유: Claude Code Auto Mode·자동화 명제가 시스템 프롬프트(1층)·RLHF(2층) 권위에 구조적으로 패배. 명제 추가는 효과 없음을 5세션 이상 자체 검증. 대신 단일 운영 모델: 사용자 명시 발화 → Claude 1턴 실행. 자세한 명제는 `feedback_authority_ladder.md` 참조.
