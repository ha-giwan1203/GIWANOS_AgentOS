...
def read_jsonl_tail(path: Path, limit: int = 5000) -> list[dict]:
    ...
    for ln in chunk:
        try:
            ...
            obj.setdefault(
                "insight", obj.get("content") or obj.get("message") or obj.get("text") or ""
            )
            # 태그 타입 보정
            tags = obj.get("tags")
...
def read_sqlite_recent(db: Path, limit: int = 1000) -> list[dict]:
    ...
    try:
        ...
        c_from = (
            "from"
            if "from" in colnames
            else ("speaker" if "speaker" in colnames else ("role" if "role" in colnames else None))
        )
        c_text = (
            "insight"
            if "insight" in colnames
            else ("content" if "content" in colnames else ("text" if "text" in colnames else None))
        )
...
def filter_rows(
    ...
    for r in rows:
        ...
        if pat:
            blob = " ".join(
                [r.get("insight", ""), r.get("from", ""), " ".join(r.get("tags") or [])]
            )
...
with src_tab:
    ...
    st.markdown("#### 데이터 소스")
    source = st.radio(
        "읽기 대상", ["JSONL(권장)", "SQLite"], horizontal=True, label_visibility="collapsed"
    )
    ...
    with cols2[1]:
        ...
        if REFL_DIR.exists():
            last = sorted(
                REFL_DIR.glob("reflection_*.json"), key=lambda p: p.stat().st_mtime, reverse=True
            )
    ...
    with cols2[2]:
        ...
        if SESS_DIR.exists():
            last = sorted(
                SESS_DIR.glob("session_*.json"), key=lambda p: p.stat().st_mtime, reverse=True
            )
    ...
    if last_minutes:
        now = datetime.now()
        span = {"5분": 5, "15분": 15, "1시간": 60, "6시간": 360, "24시간": 1440, "7일": 10080}[
            last_minutes
        ]
    ...
    with cta[1]:
        if st.button("CSV 내보내기"):
            ...
            st.download_button(
                "CSV 다운로드",
                data=data,
                file_name="velos_memory_export.csv",
                mime="text/csv",
