...
def main():
    """메인 워크플로우 함수"""
    print("🚀 VELOS Notion 완전 워크플로우 시작")
    print("=" * 50)
    ...
    if not slack_result.get("ok"):
        ...
        # Slack 실패는 치명적이지 않으므로 경고만 출력
        print("⚠️  Slack 알림 실패했지만 워크플로우는 완료되었습니다")
    else:
        print("✅ Slack 알림 성공!")
        print(f"   메시지 길이: {slack_result.get('message_length')}자")

    # 최종 결과
    print("\n🎉 VELOS Notion 워크플로우 완료!")
    print("=" * 50)
    print("📊 생성된 항목들:")
    ...
    final_result = {
        "ok": True,
        "workflow_type": "velos_notion_complete",
        "files": {
            "pdf": str(pdf_path),
            "md": str(md_path)
        },
