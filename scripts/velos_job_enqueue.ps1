...
# 추가된 작업 상세 정보 조회
cur.execute("SELECT id, kind, payload, priority, status FROM job_queue ORDER BY id DESC LIMIT 3")
recent_jobs = cur.fetchall()

con.close()

print(f"[OK] queued decide/report/notify (추가: {after_count - before_count}개)")
print(f"[INFO] 총 작업 수: {after_count}개")
print("[INFO] 최근 추가된 작업:")
for job in recent_jobs:
    print(f"  ID: {job[0]}, 종류: {job[1]}, 우선순위: {job[3]}, 상태: {job[4]}")
    print(f"  페이로드: {job[2]}")
'@

Write-Host "VELOS job_queue 작업 추가 시작..." -ForegroundColor Magenta
$result = Invoke-VelosPy -Code $enqueueCode -TimeoutSec 20 -Tag "enqueue"
Write-Host "작업 추가 결과: $result" -ForegroundColor Magenta
