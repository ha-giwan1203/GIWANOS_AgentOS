...
def get_real_memory_stats() -> Dict[str, int]:
    ...
    return {
        ...
        "total": memory_stats.get("buffer_size", 0)
        + memory_stats.get("db_records", 0)
        + memory_stats.get("json_records", 0),
    }
...
def get_real_api_costs() -> List[Dict[str, Any]]:
    ...
    for i in range(7, 0, -1):
        ...
        costs.append({"date": date, "cost": cost, "calls": 1000 + variation * 10})
    return costs
...
def get_key_improvements() -> List[Dict[str, str]]:
    ...
    if system_ok:
        improvements.append(
            {"title": "시스템 안정성 확보", "desc": "전체 시스템 상태 정상", "level": "low"}
        )
...
def get_key_risks() -> List[Dict[str, str]]:
    ...
    if total_memory > 1000:
        risks.append(
            {
                "title": "메모리 사용량 증가",
                "desc": f"총 {total_memory}개 항목으로 증가 추세",
                "level": "medium",
            }
        )
        ...
        risks.append(
            {
                "title": "메모리 누수 위험",
                "desc": f"총 {total_memory}개 항목으로 높은 사용량",
                "level": "high",
            }
        )
    ...
    if not health.get("system_integrity", {}).get("integrity_ok", True):
        risks.append(
            {"title": "시스템 무결성 문제", "desc": "파일/DB 무결성 검사 실패", "level": "high"}
        )
...
def get_weekly_changes() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "memory_usage": {
            ...
            "change": f"+{((safe_div(current_total, previous_total)-1)*100):.1f}%",
        },
...
def parse_memory_stats_from_md(md_txt: str) -> List[List[str]]:
    rows = [["항목", "값"]]
    m = re.search(
        r"메모리\s*통계:.*?버퍼\s*=\s*(\d+)\D+DB\s*=\s*(\d+)\D+JSON\s*=\s*(\d+)", md_txt, flags=re.S
    )
    if m:
...
def dispatch_rows() -> List[List[str]]:
    ...
    if d:
        rows += [
            ...
            [
                "Target",
                f"{d.get('channel') or d.get('channel_id') or ''} {d.get('database') or ''}".strip(),
            ],
        ]
    return rows


def system_meta_rows(
    health: Dict[str, Any], fonts: Dict[str, Path], sys_health: Path, autosave: Path
) -> List[List[str]]:
...
def main():
    ...
    sm = ParagraphStyle(
        name="sm", fontName="KFont", fontSize=9, leading=12, textColor=colors.HexColor("#475569")
    )
    ...
    doc = SimpleDocTemplate(
        str(out_pdf), pagesize=A4, leftMargin=36, rightMargin=36, topMargin=50, bottomMargin=36
    )
    ...
    for imp in improvements:
        imp_data.append([imp["title"], imp["desc"], imp["level"].upper()])
    ...
    for risk in risks:
        risk_data.append([risk["title"], risk["desc"], risk["level"].upper()])
...
# 표 스타일 함수들
def _summary_table_with_colors(
    rows: List[List[str]], font_body: str, data_items: List[Dict[str, str]]
) -> Table:
