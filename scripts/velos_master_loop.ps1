...
# VELOS 마스터 루프 실행 스크립트
# Python 경로를 설정하고 VELOS 마스터 루프를 실행합니다.
...
function Start-VelosMasterLoop {
    param(
        [switch]$Verbose = $false,
        [switch]$Background = $false,
        [switch]$KeepOutput = $false,
    ...
    $outputFile = if ($LogFile) { $LogFile } else { "$env:TEMP\velos_master_loop_output.txt" }
    $scriptPath = ".\scripts\run_giwanos_master_loop.py"

    Write-Host "=== VELOS 마스터 루프 실행 ==="
    ...
    try {
        ...
        if (-not (Test-Path $scriptPath)) {
            throw "마스터 루프 스크립트를 찾을 수 없습니다: $scriptPath"
        }
        ...
        if ($Background) {
            # 백그라운드 실행
            Write-Host "[BACKGROUND] 백그라운드에서 마스터 루프 실행"
            ...
            # 포그라운드 실행
            Write-Host "[FOREGROUND] 포그라운드에서 마스터 루프 실행"
            ...
            if ($process.ExitCode -eq 0) {
                Write-Host "[SUCCESS] 마스터 루프 실행 완료"

                if (Test-Path $outputFile) {
                    $outputContent = Get-Content $outputFile -Raw
                    Write-Host "`n=== 마스터 루프 출력 ==="
                    Write-Host $outputContent
                    Write-Host "=== 마스터 루프 출력 끝 ==="
                ...
                Write-Host "[ERROR] 마스터 루프 실행 실패 (Exit Code: $($process.ExitCode))"
        ...
        Write-Host "[ERROR] 마스터 루프 실행 실패: $_"
    } finally {
        # 정리 (포그라운드 실행 시에만)
        if (-not $Background) {
            if (Test-Path $outputFile) {
...
# 메인 실행
Start-VelosMasterLoop -Verbose:$Verbose -Background:$Background -KeepOutput:$KeepOutput -LogFile $LogFile

Write-Host "`n=== VELOS 마스터 루프 실행 완료 ==="
