...
def check_and_repair_db():
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            # 2. FTS 테이블 확인
            cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='memory_fts'")
            ...
            for trigger in triggers:
                cursor = conn.execute(f"SELECT name FROM sqlite_master WHERE type='trigger' AND name='{trigger}'")
            ...
            for view in views:
                cursor = conn.execute(f"SELECT name FROM sqlite_master WHERE type='view' AND name='{view}'")
                if not cursor.fetchone():
