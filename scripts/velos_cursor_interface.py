...
# VELOS 루트 경로 추가
sys.path.append(os.getenv("VELOS_ROOT", r"/home/user/webapp"))
...
class VELOSCursorInterface:
    ...
    def process_command_line(self, command: str) -> Dict:
        ...
        try:
            ...
            error_result = {"success": False, "error": f"명령 처리 중 오류 발생: {str(e)}"}
...
사용 예시:
  ...
  python velos_cursor_interface.py --status
    ...
    parser.add_argument("command", nargs="?", help="실행할 명령 (대화형 모드가 아닌 경우)")

    parser.add_argument("--interactive", "-i", action="store_true", help="대화형 모드 실행")

    parser.add_argument("--status", "-s", action="store_true", help="시스템 상태 표시")
