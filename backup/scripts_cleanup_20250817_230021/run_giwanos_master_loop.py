...
import os, sys, json, subprocess, atexit, time
    ...
    try:
        ...
        if name == "DailyReport":
            subprocess.run([sys.executable,
                            os.path.join(ROOT, "scripts", "generate_velos_report_ko.py")],
                           check=True)
        elif name == "WeeklyAudit":
            subprocess.run([sys.executable,
                            os.path.join(ROOT, "scripts", "check_velos_stats.py")],
                           check=True)
...
def run_internal_scheduler(argv: List[str]) -> None:
    ...
    if "--log-dir" in argv:
        ...
        if i+1 < len(argv): log_dir = argv[i+1]
