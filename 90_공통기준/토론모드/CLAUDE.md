...
```javascript
// Step 1: 첫 번째 대화 제목 텍스트로 요소 찾기
const items = document.querySelectorAll('.text-sm.font-medium');
const firstTitle = items[0];  // 프로젝트 채팅 목록 첫 번째

// Step 2: 상위 <a> 태그에서 href 추출 (최대 6레벨 탐색)
let node = firstTitle;
let url = null;
for (let i = 0; i < 6; i++) {
  if (!node) break;
  if (node.tagName === 'A' && node.href?.includes('/c/')) { url = node.href; break; }
