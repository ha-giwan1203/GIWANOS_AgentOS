...
허용되는 유일한 예외:
- **비가역 작업** (파일 삭제, 원본 덮어쓰기) — 이것만 사용자 확인
- 사용자가 직접 입력해야 하는 값 (URL, 주제 등)

올바른 흐름:
```
사용자 방향 지시 → Claude가 GPT에 의제 전송 → GPT 응답 비판적 분석 →
반박 전송 → 합의 도출 → 합의안 즉시 실행(파일 반영/커밋/푸시) →
완료 보고(사용자에게)
```
중간에 사용자에게 묻는 단계가 없다. 합의는 GPT와 한다. 실행은 Claude가 한다. 사용자에게는 결과만 보고한다.
