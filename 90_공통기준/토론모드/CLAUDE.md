...
## B 분류 감지 + 보고 (세션78 신설 / 세션116-117 비대칭 전환 — NEVER 생략)

2자 토론·공유 루프(`/share-result` 포함)에서 명시 트리거가 "3자"가 아니어도, 응답 내용이 아래 조건 중 하나라도 해당하면 **B 분류로 감지 + 1줄 라벨 보고 + 사용자 명시 호출 대기**. Claude 단독 판단으로 즉시 반영 금지. **자동 D(3자 토론) 진입은 차단** (세션116·117 비대칭 설계).

**B 분류 조건** — 하나라도 해당 시 (A안-1 2026-04-20 엄격화: "실행 흐름·판정 변경"만)
...
- **routine 운영(A 모드)의 결정 영역**: D0/MES/정산/잡셋업/라인배치 등 매일 반복 정형 업무에서 도메인 SKILL.md에 정의된 케이스의 처리 결정은 B 자동 승격 적용 안 함. 도메인 문서 grep으로 답이 있으면 자동 처리. 단 (1) 외부 ERP/MES 비가역 반영 직전 사전 확인, (2) hook/settings/SKILL Step 분기 변경, (3) 새 룰 추가는 본 예외에 포함 안 됨 — 기존 B 분류 유지. (세션132 3자 합의 + 외부 자료 보강: HITL escalation rate 60%+ = 시스템 미스캘리브레이션 / Claude Code Hook 과밀 부작용 외부 보고 / Graduated Trust 3계층)
...
2. **B 분류 감지 시**: 응답에 1줄 라벨 추가 — `[B 감지: <대상> — 사용자가 /debate-mode 또는 "3자 토론" 명시하면 진입]` + **수정·커밋 중단 + 사용자 명시 호출 대기**. `Skill(skill="debate-mode", ...)` 자동 호출 금지.
3. A 분류 시 기존 2자 흐름 유지 (즉시 반영 + 재커밋)
4. 사용자 지시 예외 적용 시 커밋 메시지에 예외 근거 명기 + 다음 새 의제는 다시 B/A 판정
5. **사용자가 명시 호출(예: `/debate-mode`·"3자 토론") 시에만** D 모드 진입. 자동 진입 차단.

**비대칭 설계 도입 사유 (세션116-117)**:
- 세션84(2026-04-20): B 분류 자동 승격이 너무 광범위해 "저위험도 3자 올림" 역설 → A 분류 확장 + 사용자 지시 예외 도입
- 세션116(2026-04-27): 작업 모드 5종 도입(A/B/C/D/E). 자동 D 승격은 모드 추적성을 훼손 + 모드 E(장애 복구) 영역과 모드 B(분석)·모드 C(시스템 수정) 경계 흐림
- 세션116 합의(루트 CLAUDE.md): B 감지 ON · D 자동 진입 OFF (사용자 명시 호출만 D)
- 세션117(2026-04-27): 도메인 토론모드 CLAUDE.md + share-result.md 자동 D 호출 명문 제거로 정합화. 자동 승격은 Claude 단독 판단을 막는 것이 목적이지 외부 모델을 강제로 끌어오는 절차가 아님 — 사용자 결정으로 진입 권한 이양

**왜 필요 (실증, 세션78 원래 신설 배경)**:
...
- 본 비대칭 전환은 세션78 원칙(단일 모델 단독 통과 금지)을 유지하되 진입 결정만 사용자에게 이양한다.

**[NEVER]** 사용자 지시 예외 외에 B 분류를 A로 낮춰 Claude가 단독 수정 반영 금지. B/A 판정이 모호하면 기본 B로 간주(안전측)하고 라벨 보고한다.

**[NEVER]** B 분류 감지 시 `Skill(skill="debate-mode")`를 자동 호출 금지. 사용자 명시 호출 없이 토론 모드 진입 차단 (세션117 비대칭 전환).

**관련 규정**: `.claude/commands/share-result.md` 5단계, 루트 `CLAUDE.md` "## 작업 모드 판정" 섹션, 메모리 `feedback_structural_change_auto_three_way.md`.

**합의 원본**: `90_공통기준/토론모드/logs/debate_20260427_185903_3way/` (세션116) + `90_공통기준/토론모드/logs/debate_20260427_203835_3way/` (세션117).
