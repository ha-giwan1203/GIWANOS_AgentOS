...
## 독립 검증 (NEVER — 하네스 분석 전 생략 금지)
GPT 응답이 주장/제안/지적을 포함할 때, 하네스 분석 **전에** 관련 파일을 독립적으로 읽고 내 문제 목록을 먼저 만든다.
- [MUST] `code-reviewer` 서브에이전트로 관련 파일 독립 리뷰 → `.claude/state/independent_review.md` 생성
- [MUST] 내 문제 목록과 GPT 주장을 대조한 뒤 하네스 분석 수행
- [NEVER] GPT 주장을 코드 대조만 하고 채택/보류/버림 판정하는 것은 독립 검증이 아님
- [ALLOW] GPT 응답이 단순 판정(통과/실패)이거나 보고성이면 생략 가능
