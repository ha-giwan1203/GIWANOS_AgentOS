...
### Step 2.5. 독립 검증 (하네스 분석 전 필수)
> GPT 응답을 하네스로 분석하기 **전에**, 관련 파일을 독립적으로 읽고 내 문제 목록을 먼저 만든다.
> 이 단계를 건너뛰면 GPT 프레임에 종속되어 GPT가 안 꺼낸 문제를 못 찾고, 과장을 걸러내지 못한다.

1. GPT가 언급한 파일/시스템 영역을 `code-reviewer` 서브에이전트로 독립 리뷰
2. 내 문제 목록을 `.claude/state/independent_review.md`에 기록 (세션 시작 시 초기화)
3. GPT 주장과 내 목록 대조 → 겹치는 것 / 내가 추가 발견한 것 / GPT가 과장한 것 분류
4. 이 대조 결과를 기반으로 하네스 분석 수행

> **예외**: GPT 응답이 단순 판정(통과/실패)이거나 보고성(커밋/SHA 확인)이면 독립 검증 생략 가능
