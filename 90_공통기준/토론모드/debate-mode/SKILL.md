...
#### 5-0. GPT 제안 내용 검증 (필수 — 파일 반영 전)

> GPT가 생성한 문서·규칙·코드를 기존 파일에 반영하기 전에 반드시 이 단계를 수행한다.
> 검증 없이 GPT 출력을 그대로 붙여넣는 것은 금지다.
...
| 조건부 PASS | 지적 항목만 수정 → 재커밋 → 재푸시 → Step 5-3 반복 |
| PASS + 보류 혼재 | PASS 항목만 반영 → 보류 항목은 TASKS.md 기록 → 재커밋 → 재푸시 → GPT 최종 보고 |
| HOLD (단독) | 작업 중단 → 사용자에게 보류 사유 보고 후 대기 |
| FAIL / 재검토 | 수정 후 재보고 (최대 2회) → 동일 FAIL 2회 시 사용자에게 보고 후 중단 |
