...
PASS: "즉시 적용", "바로 적용", "적용 권장", "PASS", "통과", "문제 없습니다", "진행하면 됩니다"
HOLD/FAIL: "보류", "FAIL", "재검토", "수정 필요", "문제 있습니다", "권장하지 않습니다"

### 자동 분기

- **PASS**: 사용자 확인 없이 바로 구현 → commit → push → GPT 최종 보고
- **조건부 PASS**: 수정 항목만 반영 → commit → push → GPT 재판정 요청
- **PASS+보류 혼재**: PASS만 적용 → 보류는 TASKS.md 기록 → commit → push → GPT 최종 보고
- **HOLD**: 작업 중단 → 사용자에게 보류 사유 보고 후 대기
- **FAIL/재검토**: 수정 후 재보고 (최대 2회) → 동일 FAIL 2회 시 사용자 보고 후 중단

> GPT가 PASS 판정 시 "바로 적용할까요?" 같은 확인 질문 **절대 금지**. 바로 실행.
...
> GPT 제안 반영 또는 GPT 지적(조건부 PASS/FAIL/수정 필요) 대응 시 반드시 수행.
...
- **PASS**: 루틴 종료 → 사용자 완료 보고
- **조건부 PASS**: **Step 5-0 재수행**(지적 파일 Read → 사실 여부 확인 → 맞으면 수정, 틀리면 반론) → 재커밋 → Step 5-3 반복
- **PASS+보류 혼재**: PASS 항목은 **Step 5-0 재수행** 후 반영 → 보류는 TASKS.md → 재커밋 → GPT 최종 보고
- **HOLD**: 사용자 보고 후 대기
- **FAIL/재검토**: **Step 5-0 재수행** → 수정 후 재보고 (최대 2회)
