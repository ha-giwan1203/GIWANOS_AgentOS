...
> 로컬 CDP 경로에서는 `.claude/scripts/cdp/cdp_chat_send.py --mark-send-gate`를 기본 전송 경로로 사용한다. 직접 DOM 전송은 helper를 쓸 수 없을 때만 예비 경로로 사용한다. (`--require-korean`은 deprecated no-op)
