...
### 세션79 후속 속도 개선 (1a552f55)
사용자 지적 "너무 답답한대" → 토론 모드 속도 개선 3종:
1. 응답 구조화 강제 (share-result 판정용) — verdict/item/50자
2. sleep 단축 (3→1) + polling 축소 (3/5/8 → 2/3/5)
3. 응답 읽기 {len, text} JSON 1회 반환
기각: 병렬 브라우저 창 (MCP 단일 탭 그룹 제약)
세션80 첫 토론에서 실측 검증 예정 (60~90초 → 30~40초 목표)
