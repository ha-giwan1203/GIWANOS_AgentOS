...
## 세션114 부수작업 — 센스커버 조립공정 부적합 가능성 분석

### 진행 상황
- 사용자 요청: `06_생산관리/품질/센스커버 조립공정.mp4` (89870CU100, 25초) 분석 → 엑셀 + 영상 첨부
- 1차: 자체 FMEA 9건 상세 보고서(.md + .xlsx) 작성 → 사용자 "이렇게 자세히 만들 필요 없었다" 피드백
- 2차: 사용자 제시 4건으로 단순화 — 산출물: `센스커버_조립공정_부적합가능성_대원테크.xlsx` (사용자가 v2 파일을 본인 명칭으로 채택)
- 4건 오조립 가능성: ① 스펙·색상 동일 ② 비전검사 고장→수작업 ③ 색상 판단(유사 색상) ④ 재작업 시 품번 미확인
- 영상 첨부 한계: OLE Package 임베드는 Windows packager.exe 미등록(-2146827284)으로 차단 → 썸네일 + 하이퍼링크 방식 채택 (xlsx 옆 mp4 상대 경로)

### 다음 AI 액션 (참고)
- 외부 공유 시 xlsx + mp4 두 파일을 함께 압축해 전달 권장 (하이퍼링크 상대 경로 유지)
- 1차 상세본 + 부속 분석 자료(.md, _frames_analysis/, _make_report_xlsx.py)는 백업/참고용으로 보관
...
## 세션111 (2026-04-27) — SD9A01 공정 번호 체계 변경 (10단위 + 신규 21)

### 진행 상황
- 사용자 지적: "공정 번호가 안 맞다" → AskUserQuestion으로 명확화: 산출물 파일명 (001~011 vs 10~100)
- 자료 비교: 라인별공정목록.xlsx(SD9A01)=10공정 10/20/.../100 vs ERP 화면=11공정 SD9A01_001~011
- 결정: 라인별공정목록 기준 10단위 사용 + 신규 ERP 공정에 21 부여 (작업표준서 '목록' 인덱스 21="릴브라켓 리벳팅 공정"과 일치)
- 스크립트 3건 수정 (ERP_PROCESSES, SD9M01_TO_ERP, EXPECTED_CODES) + 산출물 11+23 전량 재생성
- 자체검증 0 issues PASS / 박태순 주공정=공정21(신규) ✓

### 산출물 변경
- 파일명: `SD9A01_공정{001~011} 숙련도 평가서.xlsx` → **`SD9A01_공정{10/20/21/30/40/50/60/70/80/90/100} 숙련도 평가서.xlsx`**
- 양식 N5 셀값: `'001'~'011'` → `'10'~'100'`
- 개인별 시트명: `공정001` → `공정10` 등
...
### 다음 AI 액션 (세션111)
1. **[검증대기]** 2026-04-28 (화) 07:10 morning 자동 트리거 — OAuth 안정화 + 복원 알림 차단 효과 종합 검증
2. (폐기) morning 중복 방지책 / SmartMES 동기화 / 작업 기준 시스템 / SAC 차단 — 사용자 폐기 결정 (2026-04-27)
...
### 자체검증 + SD9A01_003 매핑 정정 (사용자 트리거: "자체검증 해서 문제점 파악후 수정")
- 검증 스크립트 신규: `verify_sd9a01_outputs.py` (A~H 8개 카테고리)
- 발견 문제: 작업표준서 시트 21 = 바코드 공정(ERP_001에 해당). 우리는 이를 ERP_003으로 잘못 사용 → SD9A01_003 양식 평가사항이 ERP 공정명("리벳/릴 하단 브라켓")과 미스매치
- 정정: 시트 매핑 변경 (001↔21 / 002↔20 / 003↔OVERRIDE) + OVERRIDE_PROCESSES 003 신설 (리벳·릴 하단 브라켓 보수적 9항목)
- 재생성 후 자체검증 **0 issues PASS**
...
### 양측 판정 (3way share, 2026-04-25)
- **GPT 판정**: `e7faa444 + 23baa33b` PASS — 전 항목 실증됨 라벨, L-4 보류 인정. 추가제안 없음(A 분류 — 추가 개선 금지). 잔여 리스크: settings.local.json 잔여 중복은 보류가 정답
- **Gemini 판정**: PASS — 전 항목 동의. L-6은 일반론 라벨, L-4는 환경미스매치 라벨(권한 차단 불가항력 수용). 추가제안 없음(A — 시스템 개선 영구 중단 + 실무 산출물 모드 전환 승인)
- 양측 합의: 모드 전환 PASS, 추가 제안 0건. 실무 산출물 모드 진입 확정
