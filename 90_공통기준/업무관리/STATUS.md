...
| hooks 체계 | 23개 등록 + `incident_repair.py` + `incident_review.py` 보조. 학습 루프 완성: 수집→탐지→제안→self-audit 통합 보고 자동화. normal_flow 분리로 진단 노이즈 제거 (세션41). evidence_gate fingerprint grace(30초 suppress) 구현 (세션43). AGENTS_GUIDE 23개/18스킬로 갱신 (세션43) |
