...
최종 업데이트: 2026-04-23 — 세션98 시스템 전체 드리프트 2자 토론 + 실행 완료 (SSoT 종결 / C2 훅 신설 / permissions 57% 감축)
...
| hooks 체계 | 31개 등록 + 스킬 18개. **기준축: `bash .claude/hooks/list_active_hooks.sh --count` (Single Source, 세션93 2자 토론 합의)**. 세션93 hook_registry.sh legacy 격하 (settings.local.json 전제 → team settings 환경 오답). 세션91 final_check statusLine 버그 수정으로 32→31 정정. 세션76 commit_gate push 단독 final_check 스킵 근본 해결. 세션74 쟁점 G 실물 분리: settings.json(TEAM) + settings.local.json(PERSONAL) + 검증 스크립트 5개 team+local union 지원. 세션72 Phase 2-B: 핵심 훅 6종 exit 2 전환 + completion_gate 소프트 블록 + timing 배선. GPT 통과 |
