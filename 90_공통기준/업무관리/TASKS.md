...
**보류 (4종)** — 실증 근거 확보 후 재검토:
- SessionStart p95 ≤ 300ms (GPT) — 라벨: **일반론**. 300ms 기준 근거 없음, hook_timing_log 실측 선행 필요
- 경고 상위 대상 구성 안정성 (GPT) — 라벨: **환경 미스매치**. 현재 STRONG이 TASKS 1건뿐, 상위 2건 이상일 때만 의미
- 증가량 추적 실효성 (GPT) — 라벨: **메타 순환**. "Phase 2 진입 지표"가 "Phase 2 자체 목적(증가량 추적)"을 되물음
- 조치 전환율 (Gemini) — 라벨: **구현 경로 미정**. "조치" 자동 감지 방법(커밋 메시지 파싱? 수동 기록?) 미제시

**판정 근거**: 외부 모델 제안 5건(GPT 3+Gemini 2) 중 실증 선례 인용 1건만 채택. 나머지는 라벨링으로 보류 — 기준값·근거·구현 경로 보강 후 재평가.
...
### [완료] debate-mode v2.11 — max_rounds 3회 확정 — 세션67 (2026-04-18)
- **v2.10 재검증 후 Gemini 반박**: "max_rounds=5는 토큰 낭비·논점 이탈·억지 타협 유발, 3회가 한계점, Fast Fail 원칙"
- **GPT 허용 가능 의견**: 5회 수용 가능하나 4~5라운드 진입 시 "재설계 필요" 라벨 권장
- **사용자 결정**: Gemini 의견 채택 → **3회 원복 확정**
- **SKILL.md v2.10 → v2.11**: `max_rounds` 기본값 5 → 3 전수 수정 (원칙/루프/자동게이트/종료판정/하네스 스키마)
- **유지 항목**: 단일 멀티턴 통일 / payload 첨부 강제 / 자동 게이트 규정 / `cross_verification` 객체 구조는 v2.10 그대로
- **변경 파일**: SKILL.md / REFERENCE.md (v2.11 이력 추가)
...
- **원인 5건**: 모델 동의 편향 / 컨텍스트 열화 / Claude 프레임(단답 우선) / 의견 영역 토론 / cowork-rules.md 미적용
- **신설 프로토콜** (단일 모델 단독 통과 금지):
  - GPT 답 → Gemini가 1줄 검증 (동의/이의/검증필요)
  - Gemini 답 → GPT가 1줄 검증
  - Claude 설계 → 양측이 1줄 검증
  - 단일 동의 ≠ 통과, 최소 2/3 검증 통과 시만 채택
- **반영 파일**:
  - `90_공통기준/토론모드/CLAUDE.md` "상호 감시 프로토콜" 섹션 신설
  - `CLAUDE.md` 외부 모델 호출 원칙에 추가
  - 메모리 `project_three_tool_workflow.md` "상호 감시 프로토콜" 섹션 신설
