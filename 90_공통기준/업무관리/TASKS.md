...
#### 토론모드 브라우저 불편사항 개선 (신규)
- 현재 방식(브라우저 조작) 유지하되 불편한 점 개선
- 개선 대상: sleep 대기 중 사용자 감지 불가, 탭 중복 생성, 대화방 누적
- MCP API 방식 전환은 보류 (GPT Git 실물 검증 기능 유지 우선)
- 참고: OpenAI API MCP 서버 방식도 대안으로 존재 (API 비용 별도, 프로젝트 컨텍스트 수동 첨부 필요)
