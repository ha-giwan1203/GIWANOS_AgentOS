...
### 미해결 — 사용자 결정 영역 (2건)
1. GPT routine/시스템 분리 vs Gemini 신뢰 위임 프로토콜 (양측 갈림, 둘 다 B 분류)
2. H12 컨텍스트 윈도우 포화 가설의 H8과 분리 여부

**[NEVER]** 단일 모델 단독 통과 금지 원칙(토론모드 CLAUDE.md L66) 따라 단독 채택 안 함. 사용자가 `/debate-mode` 명시 호출 시 Round 2 진입 가능.
