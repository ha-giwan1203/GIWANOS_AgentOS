...
Phase 2 진입 판정 지표 4종 (GPT PASS 판정 시 추가 3건 반영, 2026-04-20):
1. 주간 경고 발생 빈도 변동 < 20% (합의안 기본)
2. SessionStart 오버헤드 p95 ≤ 300ms (hook_timing 로그 기반)
3. 경고 상위 대상 구성 안정성 — 1주 동안 상위 파일 리스트 변동 작음 (대상 계속 바뀌면 임계치 재튜닝 먼저)
4. 증가량 추적 실효성 — 주간 순증가 ≥ 10% 또는 반복 강경고 + 정리 액션 부재 패턴 확인
지표 4종 미충족 시 Phase 2보다 **TASKS 감축 우선** (GPT 지적)
