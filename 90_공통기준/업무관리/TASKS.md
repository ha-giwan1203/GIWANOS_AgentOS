...
- **P1 코드 작성 완료 (사용자 명시 P1 진입 — α 결정 변경)**: `auth_extract.py` 신설 (~190줄). POST/multiList/rank/MES/DELETE 코드 0건 grep 검증. CDP_DEAD 안전 fallthrough 확인. **실측 진행 대기 — 사용자가 CDP 9223 manual launch 후 OAuth 통과 + 재실행 필요**. 결과는 `PLAN_API_HYBRID.md` Step 3 섹션에 반영 예정
