...
| auto-fix | - | - |
| chomul-module-partno | A | 초물표 모듈품번(RSP) 일괄 반영 스킬. 초물표  |
...
| night-scan-compare | B | MES 야간스캔실적 조회 → BI 대비 비교 엑셀 � |
| pptx-generator | B | - |
| production-report | B | 생산실적 집계 자동화 — BI 실적 + 임률단가 + |
| production-result-upload | A | - |
| skill-creator-merged | B | - |
| sp3-production-plan | B | SP3 생산계획 자동화 — ERP 계획 반영, D+1/D+2 � |
| supanova-deploy | A | - |
