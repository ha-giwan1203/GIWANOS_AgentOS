...
> **갱신 이력**: 2026-05-03 v2 — 3way 양측 판정(GPT PASS / Gemini 부분PASS) 보완 4건 반영 (active_skill 개별 파일화 + session_start_restore Clean-up + completion_gate error_message 규격 + Phase 0 종료조건 관측 정상성 기준).
...
- helper 함수 위치: `hook_common.sh`의 `cleanup_stale_active_skills()` 신설, `session_start_restore.sh`에서 호출
...
| 1 | d0-production-plan: SKILL이 active_skills/{task_id}.json 작성/삭제 + verify_run.py가 receipt 출력 → required:true | d0 receipt 강제 | 1주 통과율 ≥95%, override 사용 0건 |
