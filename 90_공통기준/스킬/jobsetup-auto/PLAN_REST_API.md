...
- v3.3 [완료, 2026-05-02 세션135]: 첫 서열 품번 자동 회수 — `GetProductionSchedule` API (`POST /v2/prdt/schdl/list.api`, body `{prdtDa, lineCd}`) dnSpy 추출 + 라이브 검증 3일치(4/29/4/30/5/1) PASS. `call_schedule()` + `resolve_first_sequence()` + `--auto-resolve-pno` flag + 사용자 인자 cross-check abort. spec: `state/schedule_api_spec_20260502.md`
