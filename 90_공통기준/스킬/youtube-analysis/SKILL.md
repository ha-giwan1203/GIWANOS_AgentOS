...
### Step 7 — 상태 갱신 + 커밋 + GPT 공유 + 결과 보고

**상태 갱신** (선택이 아니라 종료 조건):
1. TASKS.md: 완료 이력 추가
2. HANDOFF.md: 세션 변경사항 기록
3. STATUS.md: 갱신
4. 커밋 + 푸시
5. GPT에 커밋 SHA + 분석 결과 요약 공유
...
#### Step 3.5 — 우리 환경 대비 갭 분석

1. 영상에서 추출한 기법/구조를 현재 저장소 상태와 대조
   - hooks, rules, settings, 스킬, MCP 연동, 워크플로우 등
2. 갭 식별: 영상에는 있지만 우리에게 없는 것 / 우리가 다르게 하는 것
3. 개선안 도출: 각 갭에 대해 적용 여부 + 이유 + A/B/C 판정
4. Step 3 출력의 "우리 환경 갭 분석" 표에 반영
...
3. Drive `영상분석/raw/{video_id}/`에 원본 업로드 (`upload_to_drive.py`)

> 저장 실패 시 분석 결과는 로컬 캐시에 남아 있으므로 수동 재시도 가능

#### Step 5 — 상태 갱신 + 커밋 + GPT 공유

1. TASKS.md — 분석 완료 이력 추가
2. HANDOFF.md — 세션 변경사항 기록
3. 커밋 + 푸시
4. GPT에 커밋 SHA + 분석 결과 요약 공유
5. 사용자 보고
...
> **이 포맷은 save_to_notion.py의 PAGE_CONTENT_TEMPLATE과 동일해야 한다.**
> Claude가 Notion 저장 시 표 헤더는 고정, 행만 채운다.
