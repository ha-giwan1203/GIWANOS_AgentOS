...
# 1건 PoC (xlsm 21번째 등 미등록 후보 수동 추출 후)
python run.py --session morning --line SP3M3 --xlsx <1건xlsx> --target-date YYYY-MM-DD

# Phase 5 final_save 차단 (P3 사고 재발 방지 — rank 임시저장만)
...
| 옵션 | 동작 | 용도 |
|------|------|------|
| 기본 (--api-mode default=True) | Phase 4 rank를 requests 직접 POST (옵션 A 하이브리드) | 운영 |
| `--legacy-mode` | Phase 4를 jQuery.ajax (화면 모드) | 회귀 fallback |
| `--no-mes-send` | Phase 5 final_save 차단 (sendMesFlag='Y' MES 전송 미발생) | PoC/검증 |
| `--dry-run` | Phase 2까지만 (서버 저장 0) | 추출 검증 |
| `--parse-only` | Phase 3 selectList까지 (multiList 0) | 파싱 검증 |
| `--xlsx <path>` | Phase 1 추출 건너뛰고 외부 xlsx 그대로 업로드 | 1건 PoC, 첨부 파일 처리 |
...
### Phase 1.5: dedupe (세션133 사용자 명시 — 등록 작업 진행 전 중복 차단)
- `dedupe_existing_registrations(page, items, prod_date, line_cd)` 호출 — selectList 호출 전
- 검사: ERP 좌측 grid_body `REG_DT == prod_date AND PROD_NO 일치` → items에서 제외
- 주야간 cross 중복 허용 (다른 PLAN_DA는 검사 안 함)
- 야간 evening 1~5행 dedupe(`dedupe_night_first_5`)는 별도 (아래 16.5 참조)
- 결과 0건이면 selectList/multiList/rank/final_save 모두 스킵

### 해당일 파일 없으면 작업 패스 (세션133 사용자 명시)
- `find_plan_file()` `FileNotFoundError` 시 `[skip] 해당일 파일 없음 — 작업 패스` 출력 + exit 0 정상 종료
- `verify_run.py check_log_success`가 `skip_no_file` 마커 인식 → recover 알림 0
- 토요일/공휴일 등 자동 skip

### Phase 4: 서열 배치 (라인 공통)
- **하이브리드 모드 (기본 `args.api_mode=True`)**: `api_rank_batch(page, items, target_line, save_url, sess)` — process_one_row(dry_run=True)로 sGridList 채움 + dataList 추출 + **requests 직접 POST** `multiListMainSubPrdtPlanRankDecideMng.do` (sendMesFlag='N' 강제)
  - Content-Type: `application/json; charset=UTF-8` (P4 단계 2 발견)
  - 헤더: `ajax: true` + `X-XSRF-TOKEN` 매 호출 직전 갱신
  - mesMsg 비어있지 않으면 즉시 break (sendMesFlag='N'인데 MES 의심 차단)
- **레거시 모드 (`--legacy-mode`)**: 기존 `rank_batch` (jQuery.ajax POST) — 회귀 fallback
...
| 2026-05-01 | v4.0 | **세션133 옵션 A 하이브리드 chain 활성**. Phase 4 rank 호출 = `requests` 직접 POST (sendMesFlag='N' 강제). `args.api_mode default=True`, `--legacy-mode` fallback. Phase 5 final_save는 화면 모드 유지 — sendMesFlag='Y' MES 전송. **dedupe 사용자 명시 보강**: morning 분기 + `--xlsx direct` 분기 모두 selectList 호출 전 `dedupe_existing_registrations` 호출. 좌측 grid_body 기준 + 우측 sGridList rank 잔존 검사. 추가 가드: `--no-mes-send`(Phase 5 차단), 해당일 파일 없으면 작업 패스 (`FileNotFoundError` catch + verify_run `skip_no_file` 마커), 인접 월 fallback (월 boundary 자동 처리), `verify_run.py` `RETRY_NO/plan_path_missing` 추가 + 5xx timestamp 오매칭 차단. PoC 검증: P1 OAuth/XSRF, P2 selectList+multiList 1건, P3 rank+MES, P4 단계 1·2·3, P5 dual-mode, P6 매일 morning 자연 검증 |
