...
16.5. **야간 1~5행 dedupe** (2026-04-29 사용자 요청 — 주간과 2중 등록 방지)
   ...
   - 매칭 기준: 같은 `STD_DA`(시작일=오늘) ERP 기등록 행과 **PROD_NO 일치 AND 수량 일치**
   - 처리: 매칭된 행은 야간 등록에서 **제외** (skip=True). 나머지 행은 정상 등록
   - 데이터 소스: ERP D0 상단 그리드 GET (이미 Phase 0/3에서 로드됨) — 주간 D0 등록분이 STD_DA=오늘으로 이미 들어가 있는 상태
   - 로그: 제외된 행은 `[dedupe] 야간 1~5행 중 PROD_NO=<X> qty=<Y> 주간과 동일 → 제외` 1줄씩
...
10. **야간 1~5행 주간 중복 dedupe** (2026-04-29 사용자 요청) — Phase 4 step 16.5 참조. 야간 첫 5행 PROD_NO+수량이 같은 STD_DA 주간 등록분과 일치하면 제외. 이전엔 "주간/야간 중복 허용"이었으나 1~5행만 예외 처리
