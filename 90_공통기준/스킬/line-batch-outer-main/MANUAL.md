# line-batch-outer-main — MANUAL

> 매핑 규칙 / JS 함수 등록 코드 (OUTER_RULES / MAIN_RULES) / 실행 / 검증. SKILL.md는 호출 트리거 + 80줄 요약.
...
| 조건 | 판정 |
|------|------|
| DOM 미발견 (5회 재시도 후) | 해당 행 FAIL → `_lineErrors` 기록 |
| `setLineCell` 실패 (SELECT 미출현 또는 값 불일치) | 해당 행 FAIL (⚠️ 로그) |
| 저장 timeout (30초 초과) | 해당 행 FAIL |
| `_lineErrors` 배열에 1건 이상 존재 | 전체 결과 **부분 실패** |
| ERP 페이지 로딩 실패 / 세션 만료 | 전체 FAIL |
| topData 미존재 (그리드 데이터 없음) | 전체 FAIL |

## 중단 기준

다음 상황에서는 **즉시 중단** (`_haltLine = true`):

1. ERP 세션 만료 — 상단 그리드 데이터 소실
2. 연속 5행 이상 `DOM없음` 에러 — 페이지 구조 이상
3. 동기화 금지 구간에서 예외 발생
4. `#sub_grid_body` 그리드 미존재 — 화면 이탈
5. 사용자 수동 중지 (`_haltLine = true`)

## 검증 항목

실행 완료 후 반드시 확인:

- [ ] `lineStatus()` → done = total (전체 행 처리 완료)
- [ ] `_lineErrors` 빈 배열 확인
- [ ] 무작위 3행 ERP에서 직접 확인 — 조립라인 값 정상

**OUTER 전용:**
- [ ] row0 = `SD9A01` 고정 확인
- [ ] row1+ OUTER_RULES 매핑 일치

**MAIN 전용:**
- [ ] `LINE_DIV_NM1='MAIN'` 행만 처리됐는지 확인
- [ ] row0/row1(OUTER행) 미변경 확인
- [ ] 공정품_SUB → `SWAMSMA` / SUB → `SUB LINE 無` 매핑 일치

## 되돌리기 방법

| 범위 | 방법 |
|------|------|
| 특정 행 | ERP에서 해당 행 클릭 → SUB ASSY 조립라인을 공란으로 변경 → 저장 |
| 변경 행 추적 | `_lineLog`에서 `저장=saved` 행 인덱스 추출 가능 |
| 전체 롤백 | **이전 값 미보존** — ERP 변경 이력 조회 필요 |

> 주의: ERP 저장은 비가역적. `_lineLog`를 반드시 보존하여 변경 범위 추적 가능하게 유지할 것.
