...
## 실패 조건
- CDP 브라우저 연결 실패 (localhost:9222 미응답)
- MES 세션 만료 후 자동 로그인 실패 (auth-dev 리다이렉트 탈출 불가)
- BI 파일에 대상 날짜 데이터 0건
- 생산량(COL15) None/0 행 존재 — 데이터 미완성
- SaveExcelData.do API 500 에러 (Content-Type/iframe jQuery 미사용)
- 업로드 후 MES 조회 건수 ≠ BI 추출 건수

## 중단 기준
- iframe null 에러 → 0단계(iframe 사전 로드) 미실행 확인 후 재시도, 2회 실패 시 중단
- 중복 데이터 발견 시 사용자 확인 전까지 중단 (자동 덮어쓰기 금지)
- 생산량 빈값 발견 시 즉시 중단 → 사용자 명시 승인 없이 진행 금지
- 31일 초과 요청 시 즉시 거부

## 검증 항목
- BI 추출 건수 = MES API 업로드 응답 건수
- 업로드 후 MES 재조회: 건수 일치 + 생산량 합계 일치 (건수만으로 PASS 금지)
- 같은 날짜 데이터 분할 전송 없음 확인
- 당일 데이터 미포함 확인 (명시 요청 제외)

## 되돌리기 방법
- SaveExcelData.do는 UPSERT이므로 동일 데이터 재전송으로 덮어쓰기 가능
- 잘못된 데이터 업로드 시: 올바른 데이터로 동일 날짜 재전송 (이전 값 자동 대체)
- MES에서 수동 삭제가 필요한 경우: MES 화면에서 해당 날짜 데이터 직접 삭제 (자동화 범위 밖)
