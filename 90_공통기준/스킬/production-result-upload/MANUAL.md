# production-result-upload — MANUAL

> 0~N단계 절차 / OAuth 자동 로그인 / 데이터 품질 검증 / 실패 대응. SKILL.md는 호출 트리거 + 80줄 요약.
...
| SaveExcelData.do API 500 에러 | FAIL |
| 업로드 후 MES 조회 건수 ≠ BI 추출 건수 | FAIL |

## 중단 기준

다음 상황에서는 **즉시 중단**하고 사용자에게 보고한다:

1. iframe null 에러 → 0단계(iframe 사전 로드) 미실행 확인 후 재시도, 2회 실패 시 중단
2. 중복 데이터 발견 — 사용자 확인 전까지 중단 (자동 덮어쓰기 금지)
3. 생산량 빈값 발견 — 사용자 명시 승인 없이 진행 금지
4. 31일 초과 요청 — 즉시 거부
...
| 범위 | 방법 |
|------|------|
| 잘못된 데이터 | 올바른 데이터로 동일 날짜 재전송 (UPSERT 자동 대체) |
| 전체 날짜 롤백 | BI 원본에서 정상 데이터 재추출 → 동일 날짜 재전송 |
| MES 수동 삭제 | MES 화면에서 해당 날짜 데이터 직접 삭제 (자동화 범위 밖) |
