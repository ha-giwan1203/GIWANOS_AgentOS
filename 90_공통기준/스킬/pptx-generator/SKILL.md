...
| 템플릿 파일 읽기 실패 (경로 오류/손상) | FAIL |
| QA 3축(텍스트/시각/구조) 중 1축이라도 미통과 | FAIL |
| 생성 슬라이드 0장 | FAIL |
| matplotlib 차트 렌더링 에러 (의존성 누락) | FAIL |

## 중단 기준

다음 상황에서는 **즉시 중단**하고 사용자에게 보고한다:

1. 원본 템플릿 직접 수정 시도 감지 — 복사본에서만 작업 허용
2. shape 좌표 음수 또는 슬라이드 범위 초과 5건 이상 — 레이아웃 구조 붕괴
3. placeholder 잔존 다수 — 데이터 매핑 실패
4. 폰트 8pt 미만 다수 발생 — 가독성 문제

## 검증 항목

실행 완료 후 반드시 확인:

- [ ] QA 텍스트검사 PASS — placeholder 잔존 0건
- [ ] QA 시각검사 PASS — 겹침/잘림 0건
- [ ] QA 구조검사 PASS — 좌표/폰트/제목 기준 충족
- [ ] 슬라이드 수가 의도한 수량과 일치
- [ ] Beautify 규칙 엔진 색상 팔레트 준수
