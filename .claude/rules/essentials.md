...
| 도구 | 호출 | 시점 |
|------|------|------|
| **Codex (실행 위임)** | **`python 90_공통기준/업무관리/codex_claude_channel/auto_reply.py --target codex "<msg>"`** | **모든 실행·파일 패치·스크립트. visible 앱 창 paste 단일 채널. Agent tool `codex:codex-rescue` / `codex exec` 헤드리스 금지** |
