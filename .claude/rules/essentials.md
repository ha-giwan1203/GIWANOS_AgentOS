...
**명시 예외 (B/A 모드 유지, 경로 매칭이어도 C 미진입)**: 주석·오탈자 수정 / archive 정리 / 보고 형식·표기 정리 / 비실행 문서(`.md` doc-only, 운영 원칙 미변경) / 줄 수 무관 로직 영향 없는 문서 정리. 판정 기준은 **파일 경로**가 아니라 **실행 흐름·hook·gate·permission·데이터 변경 여부**.
