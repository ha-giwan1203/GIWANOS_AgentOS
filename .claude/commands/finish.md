...
> **종결 발화 처리** (세션157 정정 — 2026-05-13): 사용자가 "마무리"·"끝"·"끝내자"·"종료"·"완료"·"오늘 끝" 등 종결 발화 시, Claude가 작업 상태(git status·미커밋·미푸시)를 자체 판단해 본 `/finish` 루틴 사용 여부를 결정한다. **hook 자동 트리거·안내는 없다** — 세션153 `finish_trigger_detect.sh`(UserPromptSubmit hook)는 비활성 stub이며 settings.json 등록도 해제됨(2026-05-22). 종결 발화를 단순 인사로 흘리지는 않되, `/finish 진행할까요?` 같은 1줄 확인은 금지하고 자체 판단으로 처리한다. "수고했다" 단독은 인사로 받아도 된다.
>
