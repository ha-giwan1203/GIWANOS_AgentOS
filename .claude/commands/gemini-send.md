...
- **존재 (2회차 이후)** → 빠른 경로:
  1. `.claude/state/gemini_tab_id`, `.claude/state/gemini_chat_url` 읽기
  2. `tabs_context_mcp(createIfEmpty=false)` → 저장된 tabId가 현재 MCP 그룹에 존재하는지 확인
  3. 탭 존재 → **1-A/1-B 스킵 → 1-C로 직행** (foreground 활성화만)
  4. 탭 소실 → `gemini_skill_entry.ok` 삭제 → 1-A부터 전체 수행 (fallback)
- **없음 (1회차)** → 기존 1-A/1-B/1-C 전체 수행 후 **1-D에서 `.ok` 기록**

> 기존 `[NEVER] 탭 URL이 /gem/ 또는 /app/으로 보여도 1-B 생략 금지`는 **세션 간** 재사용 방지용. 세션 내 2회차 스킵은 SessionStart 훅이 매 세션 `.ok`를 무효화하므로 충돌 없음.
...
- **[NEVER]** 매 세션 1-B 최소 1회 수행 금지 해제 금지 — 세션당 첫 호출은 반드시 Gem URL navigate + 최상단 재탐지
- 같은 세션 2회차 이후 호출은 1-Z 빠른 경로로 1-A/1-B 스킵 (D안 2026-04-20)
...
1회차(`.ok` 없을 때)에 1-A/1-B/1-C를 모두 성공한 직후 `.claude/state/gemini_skill_entry.ok` 빈 파일 생성.
