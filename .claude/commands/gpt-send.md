...
3. `.claude/state/gpt_project_url` 읽기 → 해당 URL로 navigate → 3초 대기 → `javascript_tool`로 최상단 채팅방 URL 추출 → navigate
```javascript
const links = document.querySelectorAll('a[href*="/c/"]');
links.length > 0 ? links[0].href : null;
```
4. 진입 성공 시 `.claude/state/debate_chat_url` 갱신
