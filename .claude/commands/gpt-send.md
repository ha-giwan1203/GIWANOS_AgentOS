...
**2026-04-25 세션107 정책 강화 (사용자 지시)**: CDP Chrome 단독 사용. 일반 Chrome 프로필 / claude-in-chrome 계열 MCP 호출 금지. CDP 미기동 시 **0단계 자동 launch 1회 후에도 실패하면** 보고 (세션152 GPT 권고 반영 — 즉시 실패 보고 회귀 차단).
...
### 0. CDP 헬스체크 + 자동 기동 (세션152 신설 — GPT 판정 권고 별건 후속)

진입 직전 CDP 9222 헬스체크. 미기동 시 사용자 질문 없이 자동 launch 1회 시도.

```bash
curl -s --max-time 3 -o /dev/null -w "%{http_code}" http://127.0.0.1:9222/json/version
```

- HTTP 200 → 1단계 진입
- HTTP 200 외 (000 / 5xx / connect refused) → 자동 launch 1회:
  ```powershell
  ...
  ```
- 4초 후 헬스체크 재시도. HTTP 200 수신 → 1단계 진입. **2회 연속 200 미수신 시에만 [CDP 자동 기동 실패] 보고하고 호출자에게 예외 전달** (호출자가 finish_state.json terminal_state=exception, exception_reason="cdp_9222_unrecoverable" 기록).
- **[MUST]** 사용자 결정 묻지 않음. 가역·로컬·승인 불요 작업으로 분류. 세션112 사용자 지시 (2026-04-27) + 세션152 GPT 권고 (분류 A 별건 patch).
- **[NEVER]** CDP down 감지 즉시 exception 처리하고 호출자에게 차단 반환 금지 — 자동 launch 1회 우선.

상세 정책 근거: `90_공통기준/토론모드/CLAUDE.md` "CDP Chrome 단독 사용" 섹션 [MUST] 자동 기동 절차.
...
- CDP Chrome(포트 9222) 미기동 시 **0단계 자동 launch 1회 후에도 실패하면** 보고 — 기본 Chrome에서는 동작 불가 (세션152 GPT 권고 반영 — 0단계와 정합)
