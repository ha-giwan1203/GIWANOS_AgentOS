# 조립비 정산 진행 현황 (2026-05-08 세션148 4월 정산 빌더 대규모 정리 + 사용자 룰 통합)

## 현재 상태 (2026-05-08 세션148 종료 시점)
- **현재 작업월: 4월 정산** (작업폴더: `05월/` — 매월 `{MM+1}월/` 관행)
- **본체 (수식버전)**: `05월/정산_수식버전_04월.xlsx` (빌더 재실행 후 갱신, 16시트 / 미등록 영역 폐기)
- 빌더: `03_정산자동화/build_formula_version.py` (세션148 대규모 정리 — 정산 제외 룰 폐기 / 미등록 영역 폐기 / N=J·P=L 룰 정합)
- 빌더 라벨 정확성 검증: 4/4 PASS (gerp-unregistered-check 스킬)

### 라인별 합계 차이 (빌더 재실행 후)
| 라인 | 차이 | 비고 |
|------|------|------|
| SD9A01 | -7,552 | 정합 |
| ANAAS04 | +220,455 | 잔존 (다음 세션 분석) |
| DRAAS11 | 0 | |
| SP3M3 | +54,498 | RSP 정합 |
| HASMS02 | 0 | |
| HCAMS02 | -33,552 | 잔존 |
| WAMAS01 | +24,192 | 잔존 (94건 다수) |
| WABAS01 | +342,173 | 잔존 |
| WASAS01 | -23,600 | 잔존 |
| ISAMS03 | +26 | |
| **총합** | **+577,040원** | |
...
- **`05월/오류리스트_04월_GERP확인완료.xlsx`** (108건):
  ...
  - row 113: 합계
- **`05월/중복품번_오류리스트_04월.xlsx`** (52건):
  - row 5: 89830G5100WK Usage 1 중복삭제 요청
  - row 6~56: 이관품번 51건 (OVK 30 + SVM 21) — 이관품번 삭제요청
  - row 57: 합계

### 다음 세션 진입점 (남은 작업 잔존 차이 분석)
- **WAMAS01 94건 (+24,192원)** — 차이 다수, 분류·원인 분석 필요 (1순위)
- WABAS01 3건 (+342,173원) — 큰 합계 차이
- HCAMS02 6건 (-33,552원)
- SP3M3 5건 (+54,498원) — RSP 외 잔존
- ANAAS04 1건 (+220,455원)
- WASAS01 1건 (-23,600원)

### 사용자 ERP 정정 사항 (사용자 작업)
- GERP 미등록 107건 → ERP 단가 등록 (양식 J 비고 분류 참고)
- 89830R1200NNB → 웨빙 재작업 구ERP 입고 확인
- 89830G5100WK Usage 1 → GERP 단가관리 삭제 요청
- 이관품번 51건 → GERP 삭제 요청 (다음달부터 raw 미입력)
