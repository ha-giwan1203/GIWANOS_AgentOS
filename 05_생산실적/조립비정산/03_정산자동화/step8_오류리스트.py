...
for lc in LINE_ORDER:
    ...
    for r in ld.get('items', []):
        ...
        if is_excluded(r['part_no']):
            excluded.append({'line': lc, 'part_no': r['part_no'], 'diff': diff})
            continue
...
if excluded:
    ...
    print(f"  정상제외(이관품번): {len(excluded)}건 / {exc_amt:+,}원")
...
exc_info = f"  |  정상제외(이관): {len(excluded)}건({sum(e['diff'] for e in excluded):+,}원)" if excluded else ''
ws['A2'] = f"총 {len(errors)}건  |  " + '  |  '.join(summary) + exc_info
