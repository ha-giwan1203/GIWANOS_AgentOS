...
> 기준: `_pipeline_config.py` 및 `step1~step7` 소스 코드 기반 (2026-03-28 — 데이터사전 동기화 반영)
> 대상 업체: 대원테크 (vendor_cd = `0109`)
> 이 파이프라인은 검증·교차대조 보조용이다. 운영 본체는 `정산_수식버전_MM월.xlsx`이며, `정산결과_MM월.xlsx`는 step7 보조 산출본이다. step8은 오류리스트/요약 보조 산출만 담당한다.
...
9. [Step 8: 오류리스트 보조 산출](#9-step-8-오류리스트-보조-산출)
10. [Step 간 의존 관계](#10-step-간-의존-관계)
...
## 9. Step 8: 오류리스트 보조 산출

### 목적
step7 보조 산출본과 본체 검증 결과를 바탕으로 오류리스트/유형별요약을 다시 생성한다.

### 입력
- `정산결과_MM월.xlsx` (step7 보조 산출본)
- `정산_수식버전_MM월.xlsx` (운영 본체, 교차대조 기준)
- `_cache/step5_settlement.json`
- `_cache/step6_validation.json`
