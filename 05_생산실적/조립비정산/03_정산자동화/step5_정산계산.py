...
def get_olderp_pv(lc):
    ...
    elif li.get('type') in ('OUTER', 'MAIN'):
        return (olderp_line_day.get(lc, {}),
                olderp_line_night.get(lc, {}),
                olderp_line_total.get(lc, {}))
    else:
...
def calc_night_price(lc, price, night_qty):
    ...
    SP3M3: qty × 170원 고정단가
    SD9A01(단가≤500): qty×단가×30% (야간가산: 30%만, 기본100%는 day_amt에 이미 포함)
    SD9A01(단가>500): qty×단가 (기본, 야간수당 없음)
    기타 야간 없는 라인: 0
    """
    if night_qty == 0:
        return 0
    ...
    if lc == 'SD9A01':
        return round(night_qty * price * 0.3)  # 야간수당: 기준단가×30% (GERP 추가 행 구조)
    return 0
...
for lc in LINE_ORDER:
    ...
    for r in rows:
        ...
        # 금액 계산
        g_day_amt = round(g_day_qty * price)
