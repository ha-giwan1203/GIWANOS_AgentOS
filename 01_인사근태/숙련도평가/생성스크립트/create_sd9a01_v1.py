...
# ─── ERP 11공정 매핑 (사용자 화면 캡처 기준, 자체검증 후 정정) ──────────────
# (erp_no, erp_name, merged_sheet, std_filter, default_level)
# std_filter: 'all' = 시트 자주검사 전체 / 'override' = OVERRIDE_STD 사용 (작업표준서 시트 미매핑)
#
# 매핑 근거:
#   - 시트 20  = 어퍼스테이 압입/실드 조립/WEBBING GUIDE  → ERP_002 (메인) + ERP_001(바코드 부착상태 항목 1건만 보유)
#   - 시트 21  = 프레임 바코드/2D 바코드/와이어 조립      → ERP_001 PACKAGE 바코드 부착에 해당 (자체검증 정정)
#   - 시트 30  = 웨빙 삽입                                → ERP_004
#   - 시트 40  = 틸트락 검사                              → ERP_005
#   - 시트 50  = 부착구(TONGUE/D-RING) 조립              → ERP_006
#   - 시트 60  = 앵커 접이                                → ERP_007
#   - 시트 70  = 미싱                                     → ERP_008 (라인구분 변경: 컷이송→실미싱)
#   - 시트 80  = 텅스토퍼 압입                            → ERP_009
#   - 시트 90  = 최종 검사                                → ERP_010
#   - 시트 100 = 포장                                     → ERP_011
#   - ERP_003 (RETRACTOR MAIN FRAME 서브 앗세이 압입 & 리벳 & 릴 하단 브라켓):
#     작업표준서에 직접 매칭되는 시트 없음 → OVERRIDE_STD/CTRL 사용 (사용자 검수 필요)
ERP_PROCESSES = [
    ('001', 'PACKAGE 바코드 부착',                                                  '21', 'all',      3),
    ('002', 'RETRACTOR MAIN FRAME 릴 상단 브라켓 압입 & WEBBING GUIDE',           '20', 'all',      3),
    ('003', 'RETRACTOR MAIN FRAME 서브 앗세이 압입 & 리벳 & 릴 하단 브라켓',     None, 'override', 3),
...
# SD9A01_003 전용 override (작업표준서 미매핑 — ERP 공정명 기반 보수적 텍스트)
OVERRIDE_PROCESSES = {
    '003': {
        'std': [
...
def build_processes() -> list:
    ...
    for (erp_no, erp_name, sheet_no, std_filter, level) in ERP_PROCESSES:
        if std_filter == 'override' and erp_no in OVERRIDE_PROCESSES:
            std = OVERRIDE_PROCESSES[erp_no]['std']
            ctrl = OVERRIDE_PROCESSES[erp_no]['ctrl']
        else:
