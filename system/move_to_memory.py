...
BASE_DIR = "C:/giwanos"
TARGET_FOLDER = os.path.join(BASE_DIR, "memory")
FILE_PATTERNS = ["*.json", "*.db", "*.sqlite3"]

def move_files_to_memory(base_dir, target_folder, patterns):
...
def main():
    print("📂 'memory' 폴더로 (.json, .db, .sqlite3) 파일 자동 정리 시작...\n")

    moved_files, errors = move_files_to_memory(BASE_DIR, TARGET_FOLDER, FILE_PATTERNS)
